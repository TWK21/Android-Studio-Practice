...
implements SensorEventListener {
    ...
    private String[] words = new String[]{"Key Club", "Robotics", "CSF", "Armenian Club", "Prom Plus", "Spanish Club", "NHS", "Baking Club", "Chemistry Club", "Ping Pong Club"};
                ...
                while (!this.isInterrupted()) {
                    try {
                        ...
                        ClubActivity.this.runOnUiThread(new Runnable(){
                             ...
                             */
                            public void run() {
                                if (ClubActivity.this.time != 0) {
                                    ClubActivity clubActivity = ClubActivity.this;
                                    clubActivity.time = -1 + clubActivity.time;
                                } else if (ClubActivity.this.time == 0) {
                                    ClubActivity.this.wordTxt.setText((CharSequence)("TIME IS OVER! Score: " + String.valueOf((int)ClubActivity.this.score)));
                                }
                                ClubActivity.this.timeTxt.setText((CharSequence)String.valueOf((int)ClubActivity.this.time));
                            }
                        });
                    }
