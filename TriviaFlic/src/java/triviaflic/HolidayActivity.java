...
implements SensorEventListener {
    ...
    private String[] words = new String[]{"Thanksgiving", "Martin Luther King Jr.", "Christmas", "Veterans Day", "Lincoln's Birthday", "Memorial Day", "Graduation", "Finals Week", "Armenian Genocide Remembrance", "Student Holiday"};
                ...
                while (!this.isInterrupted()) {
                    try {
                        ...
                        HolidayActivity.this.runOnUiThread(new Runnable(){
                             ...
                             */
                            public void run() {
                                if (HolidayActivity.this.time != 0) {
                                    HolidayActivity holidayActivity = HolidayActivity.this;
                                    holidayActivity.time = -1 + holidayActivity.time;
                                } else if (HolidayActivity.this.time == 0) {
                                    HolidayActivity.this.wordTxt.setText((CharSequence)("TIME IS OVER! Score: " + String.valueOf((int)HolidayActivity.this.score)));
                                }
                                HolidayActivity.this.timeTxt.setText((CharSequence)String.valueOf((int)HolidayActivity.this.time));
                            }
                        });
                    }
